...
public sealed partial class DatabaseSeeder
{
    ...
    private async Task ExecutarSeedAsync(CancellationToken cancellationToken)
    {
        ...
        // TENANT DEFAULT
        // =============================================
        var tenantDefault = Tenant.Criar("Sistema Demo", "demo@ordemservico.com", "123.456.789/0001-00", "(11) 98765-4321", "Rua Demo, 123", "Sao Paulo", "SP", PlanoTenant.Enterprise);
        await _context.Tenants.AddAsync(tenantDefault, cancellationToken);
        await _context.SaveChangesAsync(cancellationToken);

        // =============================================
        // CLIENTES
        // =============================================
        var maria = Cliente.Criar(tenantDefault.Id, "Maria Silva", "123.456.789-00", "(11) 99876-5432", "maria.silva@email.com", "Rua das Flores, 123 - Centro, Sao Paulo/SP");
        var joao = Cliente.Criar(tenantDefault.Id, "Joao Santos", "987.654.321-00", "(21) 98765-4321", "joao.santos@gmail.com", "Av. Brasil, 456 - Copacabana, Rio de Janeiro/RJ");
        var ana = Cliente.Criar(tenantDefault.Id, "Ana Costa Oliveira", "456.789.123-00", "(31) 97654-3210", "ana.costa@hotmail.com", "Rua Minas Gerais, 789 - Savassi, Belo Horizonte/MG");
        var carlos = Cliente.Criar(tenantDefault.Id, "Carlos Ferreira", "321.654.987-00", "(41) 96543-2109", "carlos.ferreira@empresa.com.br", "Rua XV de Novembro, 321 - Centro, Curitiba/PR");
        var lucia = Cliente.Criar(tenantDefault.Id, "Lucia Rodrigues", "654.321.987-00", "(51) 95432-1098", null, "Av. Ipiranga, 654 - Partenon, Porto Alegre/RS");
        var pedro = Cliente.Criar(tenantDefault.Id, "Pedro Almeida ME", "12.345.678/0001-90", "(11) 3456-7890", "contato@pedroalmeida.com.br", "Rua Augusta, 1500 - Consolacao, Sao Paulo/SP");
        ...
        // EQUIPAMENTOS
        // =============================================
        var notebookMaria = Equipamento.Criar(tenantDefault.Id, maria.Id, "Notebook", "Dell", "Inspiron 15 5000", "SN-DELL-2024-001");
        var celularMaria = Equipamento.Criar(tenantDefault.Id, maria.Id, "Celular", "Samsung", "Galaxy S24", "IMEI-SAM-2024-555");

        var notebookJoao = Equipamento.Criar(tenantDefault.Id, joao.Id, "Notebook", "Apple", "MacBook Pro 14\"", "SN-APPLE-C02X1234");
        var impressoraJoao = Equipamento.Criar(tenantDefault.Id, joao.Id, "Impressora", "HP", "LaserJet Pro M404dn", "SN-HP-LJ-78901");

        var desktopAna = Equipamento.Criar(tenantDefault.Id, ana.Id, "Desktop", "Lenovo", "ThinkCentre M90q", "SN-LEN-TC-45678");
        var monitorAna = Equipamento.Criar(tenantDefault.Id, ana.Id, "Monitor", "LG", "UltraWide 34WN80C", "SN-LG-MON-11223");

        var notebookCarlos = Equipamento.Criar(tenantDefault.Id, carlos.Id, "Notebook", "Asus", "VivoBook S15", "SN-ASUS-VB-99887");

        var celularLucia = Equipamento.Criar(tenantDefault.Id, lucia.Id, "Celular", "Apple", "iPhone 15 Pro", "IMEI-APPLE-2024-777");

        var servidorPedro = Equipamento.Criar(tenantDefault.Id, pedro.Id, "Servidor", "Dell", "PowerEdge T340", "SN-DELL-PE-44556");
        var notebookPedro = Equipamento.Criar(tenantDefault.Id, pedro.Id, "Notebook", "HP", "EliteBook 840 G9", "SN-HP-EB-33445");
        ...
        var os1 = OrdemServico.Criar(tenantDefault.Id, maria.Id, notebookMaria.Id,
        ...
        os1.AdicionarAnotacao("Cliente autorizou reparo por telefone. Informou que pode retirar a partir de.", Guid.Empty, "Sistema Demo");
        os1.AdicionarAnotacao("Placa-mae encomendada. Previsao de chegada: 2 dias uteis.", Guid.Empty, "Sistema Demo");
        ...
        var os2 = OrdemServico.Criar(tenantDefault.Id, joao.Id, notebookJoao.Id,
        ...
        os2.AdicionarAnotacao("Orcamento enviado por e-mail ao cliente. Aguardando aprovacao.", Guid.Empty, "Sistema Demo");
        os2.MarcarComoOrcamento();

        // --- OS 3: Ana - Desktop Lenovo - Em Andamento ---
        var os3 = OrdemServico.Criar(tenantDefault.Id, ana.Id, desktopAna.Id,
        ...
        os3.AdicionarAnotacao("Teste de stress com Prime95 confirmou falha em memoria slot B. Modulo com defeito fisico.", Guid.Empty, "Sistema Demo");
        os3.MarcarComoOrcamento();
        os3.Aprovar();
        os3.IniciarAndamento();

        // --- OS 4: Carlos - Notebook Asus - Rascunho ---
        var os4 = OrdemServico.Criar(tenantDefault.Id, carlos.Id, notebookCarlos.Id,
        ...
        var os5 = OrdemServico.Criar(tenantDefault.Id, lucia.Id, celularLucia.Id,
        ...
        os5.AdicionarAnotacao("Bateria trocada com sucesso. Saude 100%. Aguardando retirada pela cliente.", Guid.Empty, "Sistema Demo");
        ...
        var os6 = OrdemServico.Criar(tenantDefault.Id, pedro.Id, servidorPedro.Id,
        ...
        os6.AdicionarAnotacao("Cliente solicitou manutencao para sabado. Peca encomendada.", Guid.Empty, "Sistema Demo");
        os6.AdicionarAnotacao("Fan substituido com sucesso. Disco RAID aguardando peca (HD SAS).", Guid.Empty, "Sistema Demo");
        ...
        var os7 = OrdemServico.Criar(tenantDefault.Id, joao.Id, impressoraJoao.Id,
        ...
        os7.AdicionarAnotacao("Cliente informou que vai comprar impressora nova. Orcamento rejeitado.", Guid.Empty, "Sistema Demo");
        os7.MarcarComoOrcamento();
        os7.Rejeitar();

        // --- OS 8: Maria - Celular Samsung - Rascunho (sem itens) ---
        var os8 = OrdemServico.Criar(tenantDefault.Id, maria.Id, celularMaria.Id,
